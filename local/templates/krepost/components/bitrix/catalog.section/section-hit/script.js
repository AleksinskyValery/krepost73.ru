...
function toggleSelect({
   ...
   selectBody.addEventListener('click', (e) => {
       if (e.target && e.target.classList.contains('select__item')) {
           ...
           if (selectCurrent.textContent === 'цена по возрастанию') {
               window.history.pushState('1', '', '?sort=catalog_PRICE_1&method=ASC');
           }
           if (selectCurrent.textContent === 'цена по убыванию') {
               window.history.pushState('1', '', '?sort=catalog_PRICE_1&method=DESC');
           }
