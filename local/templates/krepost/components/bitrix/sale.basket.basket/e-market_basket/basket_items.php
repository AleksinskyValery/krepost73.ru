			...
			<thead class="basket_header">
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						if ($arHeader["id"] == "NAME"):
						?>
							<td class="items col-xs-6" colspan="2" id="col_<?=$arHeader["id"];?>">
						<?
						elseif ($arHeader["id"] == "PRICE"):
						?>
							<td class="price col-xs-2" id="col_<?=$arHeader["id"];?>">
						<?
						else:
						?>
							<td class="custom col-xs-2" id="col_<?=$arHeader["id"];?>">
						<?
						endif;
						?>
							<?=$arHeader["name"]; ?>
				...
				?>
					<tr class="item" id="<?=$arItem["ID"]?>">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
								...
								<td class="item_info">
									...
									<h2 class="bx_ordercart_itemtitle">
                                      ...
                                      <? if($arItem["PROPERTY_EMARKET_PREVIEW_CH_VALUE"]): ?>
                                             <div class="prew_char"><?=$arItem["PROPERTY_EMARKET_PREVIEW_CH_VALUE"]?></div>
                                      <? endif; ?>
															...
															<ul id="prop_<?=$arProp["CODE"]?>_<?=$arItem["ID"]?>"
																...
																?>
																	<li
                                                                        ...
                                                                        <a href="javascript:void(0)" class="cnt"><span class="cnt_item"><?=$arSkuValue["NAME"];?></span></a>
																	</li>
																<?
																endforeach;
																?>
									...
									<div class="centered">
										<table cellspacing="0" cellpadding="0" class="counter">
											<tr>
												...
												{
													?>
														<td style="text-align: left; display: none;"><?=$arItem["MEASURE_TEXT"]?></td>
													<?
												}
												?>
											</tr>
							...
							?>
								<td class="custom sum">
									<span class="txt"><?=$arHeader["name"]; ?>:</span>
									<?
									if ($arHeader["id"] == "SUM"):
									?>
										<span class="summ" id="sum_<?=$arItem["ID"]?>">
									<?
									endif;
						...
						?>
							<td class="control">
								...
								if ($bDeleteColumn):
								?>
									<a class="remove mdi mdi-close" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"></a
								<?
								endif;
								if ($bDelayColumn):
								?>
	...
	<div class="bx_ordercart_order_pay">
		...
		?>
			<div class="bx_ordercart_coupon">
				<input type="text" id="coupon" name="COUPON" placeholder="<?=GetMessage("STB_COUPON_PROMT")?>" value="" onchange="enterCoupon();"><a class="btn btn_green mdi mdi-chevron-right" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"></a>
			</div><?
				if (!empty($arResult['COUPON_LIST']))
				{
					foreach ($arResult['COUPON_LIST'] as $oneCoupon)
					{
						...
						?><div class="bx_ordercart_coupon">
							<input disabled readonly type="text" name="OLD_COUPON[]" value="<?=htmlspecialcharsbx($oneCoupon['COUPON']);?>" class="cupon_input <? echo $couponClass; ?>">
								<span class="<? echo $couponClass; ?>" data-coupon="<? echo htmlspecialcharsbx($oneCoupon['COUPON']); ?>">
								</span>
...
?>
			...
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					<tr>
						...
						<td class="fwb" id="allSum_FORMATED"><span><?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?></span></td>
					</tr>
		...
		<div class="bx_ordercart_order_pay_center">
                    <!-- <a class="btn btn_white" ?><?=GetMessage("SALE_CLEAR") ?></a> !-->
			...
			{
				?>
				<a href="javascript:void(0)" onclick="updateBasket();" class="btn btn_blue checkout refresh"><?=GetMessage("SALE_REFRESH")?></a>
				<?
			}
			?>
			<a href="javascript:void(0)" onclick="checkOut();" class="btn btn_blue "><?=GetMessage("SALE_ORDER")?></a>
...
?>
<p style="padding: 16px;"><?=GetMessage("SALE_NO_ITEMS");?></p>
<?
endif;
?>