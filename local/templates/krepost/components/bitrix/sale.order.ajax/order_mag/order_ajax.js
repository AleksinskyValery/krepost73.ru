	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
			...
			if (currentSection && currentSection.id.indexOf(firstSection.id) == '-1')
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: '',
							className: 'pull-left btn btn_blue'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: '', className: 'pull-right btn btn_blue'},
		...
		createBasketItemContent: function(data)
		{
			var itemName = data.NAME || '',
				...
				props = data.PROPS || [],
				propsNodes = [];
                                //prevHtml = this.htmlspecialcharsEx(data.PROPERTY_EMARKET_PREVIEW_CH_VALUE);

			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
				titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + '</a>';                                
                                
                        //if (data.PROPERTY_EMARKET_PREVIEW_CH_VALUE)
                        //    prevHtml = '<span>' + data.PROPERTY_EMARKET_PREVIEW_CH_VALUE + '</span>';
			...
			return BX.create('DIV', {
				...
				children: propsNodes.length ? [
					BX.create('DIV', {props: {className: 'bx-soa-item-title'}, html: titleHtml}),
                                       // BX.create('DIV', {props: {className: 'bx-soa-item-preview'}, html: prevHtml}),
		...
		createPaySystemItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-3 col-md-4 col-sm-6 col-xs-3'},
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-xs-9 bx-soa-pp-company-selected'},
						children: arNodes
					})
				);
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-xs-3 bx-soa-pp-price'},
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'btn btn_blue btn-order-save'
								},
...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getDeliveryPriceNodes: function(delivery)
		{
			...
			console.log(delivery);
			if (typeof delivery.DELIVERY_DISCOUNT_PRICE !== 'undefined'
				&& typeof delivery.PRICE !== 'undefined' && delivery.PRICE >=0
				&& parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(delivery.PRICE))
			{
				if (parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) > parseFloat(delivery.PRICE))
					priceNodesArray = [delivery.DELIVERY_DISCOUNT_PRICE_FORMATED];
